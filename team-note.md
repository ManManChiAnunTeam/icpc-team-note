...
### powergee's Convex Hull & Rotating Calipers
```C++
#include <iostream>
#include <vector>
#include <algorithm>
...
Long calcDist(const Point& p1, const Point& p2)
{
    return (p1.X - p2.X) * (p1.X - p2.X) + (p1.Y - p2.Y) * (p1.Y - p2.Y);
}

Long calcCCW(const Point& p1, const Point& p2, const Point& p3)
{
    return p1.X * p2.Y + p2.X * p3.Y + p3.X * p1.Y
        - p1.Y * p2.X - p2.Y * p3.X - p3.Y * p1.X;
}

Long calcCCW(const Point& v1, const Point& v2)
{
...
{
    Long ccw = calcCCW(anglePivot, p1, p2);
    if (ccw > 0)
        return true;
    else if (ccw == 0)
        return calcDist(anglePivot, p1) < calcDist(anglePivot, p2);
    else return false;
}
...
{
    int size = hull.size();
    Long ccw = calcCCW(hull[size - 2], hull[size - 1], p);

    while (ccw < 0)
    {
        hull.pop_back();
        size = hull.size();
        ccw = calcCCW(hull[size - 2], hull[size - 1], p);
    }
    ...
    {
        if (calcDist(hull[size - 2], p) > calcDist(hull[size - 2], hull[size - 1]))
            hull[size - 1] = p;
    }
}
...
std::pair<Point, Point> getFarthest(std::vector<Point>& hull)
{
    ...
    for (int i = 0; i < (int)hull.size()*2; ++i)
    {
        ...
        double relCCW = calcCCW(hull[currP1] - hull[p1], hull[p2] - hull[currP2]);
